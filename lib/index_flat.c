 ...
 */
static void insert_node(INodeFlat **head, INodeFlat *node) {
    node->next = *head;
    node->prev = NULL;
...
static INodeFlat *search_node(INodeFlat **head, uint64_t id) {
 ...
 */
static int delete_node(INodeFlat **head, uint64_t id) {
...
static int flat_search_n(void *index, float32_t *vector, uint16_t dims, MatchResult **result, int n) {
    ...
    *result = (MatchResult *) calloc_mem(n, sizeof(MatchResult));
    ...
    while (current) {
        distance = idx->cmp->compare_vectors(current->vector->vector, v, idx->dims_aligned);
        for (k = 0; k < n; k++) {
            if (idx->cmp->is_better_match(distance, (*result)[k].distance)) {
...
static int flat_search(void *index, float32_t *vector, uint16_t dims, MatchResult *result) {
    ...
    while (current) {
        distance = idx->cmp->compare_vectors(current->vector->vector, v, idx->dims_aligned);
        if (idx->cmp->is_better_match(distance, result->distance)) {
